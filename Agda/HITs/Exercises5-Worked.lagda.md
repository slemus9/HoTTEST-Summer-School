...
module Exercises5-Worked where
```
...
rev-equiv = Inverse rev h rev h
 where
  h : (rev ∘ rev) ∼ id
  h = S1-elim (λ x → (rev ∘ rev) x ≡ x) loop (PathOver-roundtrip≡ rev rev loop  H)
   where
    H : ! loop ∙ (ap rev (ap rev loop) ∙ loop) ≡ loop
    H = ∙assoc (! loop) _ loop ∙
        ap (λ p → ! loop ∙ (ap rev p) ∙ loop) (S1-rec-loop base (! loop)) ∙
        ap (λ p → (! loop) ∙ p ∙ loop) (ap-! loop) ∙
        ap (λ p → (! loop) ∙ (! p) ∙ loop) (S1-rec-loop base (! loop)) ∙
...
PathOver-path≡ : ∀ {A B : Type} {g : A → B} {f : A → B}
                        ...
                        → (q ∙ ap g p ≡ ap f p ∙ r )
                        → q ≡ r [ (\ x → (f x) ≡ (g x)) ↓ p ]
PathOver-path≡ {A}{B}{g}{f}{a}{a}{refl _}{q}{r} h =
